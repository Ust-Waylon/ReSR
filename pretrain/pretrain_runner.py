...
def prepare_batch(batch, device):
    inputs, labels = batch
    inputs_gpu = th.stack([th.LongTensor(x) for x in inputs]).to(device)
...
class PretrainRunner:
    ...
    def train(self, epochs, log_interval=100):
        ...
        t = time.time()
        mean_loss = 0
        enable_amp = True if "cuda" in self.device.type else False
        # enable_amp = False
        if enable_amp:
            scaler = th.amp.GradScaler(enabled=enable_amp)
        for epoch in range(epochs):
            ...
            for batch in self.train_loader:
                ...
                if enable_amp:
                    with th.autocast(device_type=self.device.type, enabled=enable_amp):
